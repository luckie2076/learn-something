// 阶段二：用 TS 写源码（Webpack 用 ts-loader 转译）。
// 阶段三：下面两行分别引入「全局 Sass」和「CSS Modules 化的 Sass」。
import './style.scss';
import styles from './style.module.scss';
// 阶段四：import 非 JS 资源 —— 由 Webpack 的 loader 在打包时转成 JS 模块。
import logoUrl from './logo.png'; // 默认导出：该图片在产物里的 URL（小图被内联成 data URL）
import info from './data.json'; // 默认导出：解析后的 JS 对象

const app = document.querySelector<HTMLDivElement>('#app')!;
app.className = styles.card; // CSS Modules：styles.card 是构建时被哈希化的唯一类名

// import 进来的 png：拿到的是 URL（小图被 Webpack 内联成 base64 data URL），直接喂给 <img>
